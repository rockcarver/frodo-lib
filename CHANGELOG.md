...
- Outbound HTTP/HTTPS connection keep-alive (`BaseApi.ts`): The shared module-level agents and per-call scoped variants now reuse outbound TCP/TLS connections between axios calls, reducing connection churn and TIME_WAIT/ephemeral-port pressure under load. Set `FRODO_NO_KEEPALIVE=1` to restore the old per-request connection behavior. (#636, #637)

- Config writers are now self-sufficient for bare library consumers that never call `initConnectionProfiles()`/`initTokenCache()`. They create the target file's directory before writing, preventing confusing ENOENT errors. Token-cache reads treat a missing cache file as an empty cache rather than failing the whole save. (#636, #637)
- The `DataProtection` master-key bootstrap no longer silently encrypts under an empty key. If a new master key cannot be written, the error is rethrown instead of being swallowed, preventing the use of a degenerate key. Callers see the failure through their existing semantics. (#636, #637)
...
- Added `state.setColorTheme(theme: 'dark' | 'light')`/`state.getColorTheme()`, mirroring the existing `setPrintHandler`/`setErrorHandler`/etc. pattern.
- Added best-effort terminal-background-color detection (`TerminalBackgroundDetection.ts`): `detectTerminalBackgroundRgb` queries the terminal's actual background via OSC 11 (short timeout, guaranteed raw-mode/listener cleanup, skipped entirely on a non-TTY invocation), and `matchBackgroundPreset` does simple nearest-neighbor matching of a detected RGB against a caller-supplied set of named reference backgrounds. Both exported from the package root for frodo-cli's terminal-background auto-detection feature.
...
- Added `frodo.idm.managed.schema.createManagedObjectSchemaFlatProperty`/`updateManagedObjectSchemaFlatProperty`/`removeManagedObjectSchemaFlatProperty` for flag-driven, non-relationship schema property CRUD (including nested sub-properties, enumerations, default values, and script-derived/relationship-derived virtual properties), available on any deployment that runs IDM via the generic `readSubConfigEntity`/`importSubConfigEntity` whole-type path. Also exports the supporting `MANAGED_OBJECT_SCHEMA_CREATABLE_PROPERTY_TYPES` constant/`ManagedObjectSchemaCreatablePropertyType` type (the curated, UI-facing subset of property types, distinct from the existing wire-format `ManagedObjectSchemaPropertyType`), the pure `buildManagedObjectSchemaPropertyPayload`/`extractManagedObjectSchemaPropertyFields` functions (now validating property type and matching `enumValues`/`enumTitles` lengths, throwing `FrodoError`), and the sub-property path helpers (`PropertyContainer`, `navigatePropertyPath`, `navigateToPropertyContainer`, `parseSubPropertyPath`, `setSchemaProperty`, `removeSchemaProperty`) — migrated out of frodo-cli, which had built this logic locally and is now a thin consumer.
- Added `frodo.idm.managed.schema.createManagedObjectType`/`updateManagedObjectType`/`removeManagedObjectType` for managed-object type-level (title/icon/description) create/update/remove, using the same generic `readSubConfigEntity`/`importSubConfigEntity`/`removeSubConfigEntity` whole-type path as the flat-property functions above. `createManagedObjectType` refuses if the type already exists; `updateManagedObjectType` refuses if it doesn't and returns `{ current, proposed }` for a caller-built confirmation diff; `removeManagedObjectType` relies on `removeSubConfigEntity`'s own not-found error rather than a redundant existence read. Also exports the supporting `ManagedObjectTypeFields` type, the pure `buildManagedObjectTypeSchema` function, and the `MANAGED_OBJECT_TYPE_DEFAULT_ICON` constant (`'widgets'`, a Material Design Icon name, matching that these types only ever target Cloud/ForgeOps deployments) — migrated out of frodo-cli the same way as the flat-property functions.
...
- The MCP capability registry's naming-convention inference only recognizes `delete*`-prefixed methods as delete operations, but this session's ten new `frodo.idm.managed.schema.*` schema-mutation methods (the flat-property, type-level, and relationship-property functions added above) follow this codebase's established `create*`/`update*`/`remove*` convention, not `delete*`. Left unclassified, the three `remove*` ones fell through to `operationType: 'special'`, `riskClass: 'low'`, `mutating: false` — despite deleting part of a type's schema, IDM state affecting every existing and future record — which let them slip past the `agentic`/`standard` MCP policy presets' `denyOperationTypes: ['delete', ...]`/`denyRiskClasses: ['critical']` filters (both presets are explicitly designed to withhold destructive capabilities from less-trusted profiles); the naming-convention default for `create`/`update` methods (`riskClass: 'medium'`) was too permissive too, since the existing sibling v2-API functions in this same domain are deliberately elevated to `riskClass: 'critical'` for the same reason. Added explicit `CapabilityMetadata.ts` entries for all ten (`operationType: 'delete'` plus `riskClass: 'critical'` for the three `remove*` ones; `riskClass: 'critical'` for the other seven), matching the existing `idm.managed.schema.updateManagedObjectSchemaProperty`/`removeManagedObjectSchemaProperty` entries' own precedent and reasoning — verified live against `buildCapabilityInventory`/`applyCapabilityPolicy` that all nine mutating capabilities are now excluded from `read-only`/`agentic`/`standard` and reachable only under `admin`, while the one read-only addition (`readManagedObjectSchemaRelationshipPropertyOrNull`) correctly remains available everywhere.
- `cloud.secret.pruneVersionsOfSecret` had `mutating: false`/`destructive: false` despite permanently deleting old secret versions — another instance of the same naming-convention gap (no `create`/`update`/`delete`/etc. prefix to infer from). Exposure itself was already safe by coincidence (its `secret`-keyword risk-class match already excludes it from `read-only`/`agentic`/`standard`), but the metadata was still wrong; added an explicit entry.
- Added a permanent regression guardrail (`CapabilityRegistry.test.ts`'s `getUnreviewedSpecialCapabilities`) that fails CI the moment any `'special'`-kind capability — library-wide, any domain — has no explicit `CapabilityMetadata.ts` entry at all, rather than silently inheriting the permissive `mutating: false`/`riskClass: 'low'` naming-convention default. This is the enforcement mechanism the "Special-capability audit (library-wide)" section's own comment claimed already existed (attributed to the `contract-gap-baseline.json`/`getHighRiskMixedContractViolations` guardrail) but didn't: that guardrail only checks `argumentMode: 'mixed'` capabilities with zero derived parameters, restricted to `create`/`update`/`read`/`search`/`delete` operation types across four domains — it explicitly excludes `'special'`-kind capabilities from its filter and so could never have caught this class of gap, including the ten `idm.managed.schema.*` methods and `cloud.secret.pruneVersionsOfSecret` above. Zero-tolerance (`toEqual([])`), not a baseline diff: the Aug 2026 audit already closed this gap for every capability that existed then, so the bar is zero going forward too, not "no worse than a known backlog."
...
- The MCP capability registry now derives parameter contracts from the Help.ts catalog as a fallback when no explicit capability metadata override exists, closing several previously-unconstrained mixed-argument capabilities automatically.
...
- Updated console color rendering to use `tinyrainbow` bright variants in default error and diagnostic paths, improving readability of CLI-integrated library output without changing API behavior (PR #606).

### Fixed

- Fixed IGA workflow export pagination across tenants with different paged-results behavior by normalizing query offset handling and making governance search aggregation robust to null `totalCount`/`totalHits` values (PR #607).
- Fixed node orphan discovery/removal reliability by hardening node-version resolution across mixed node payload shapes, deduplicating orphan candidates by node ID, improving active/total/orphan count consistency, adding richer delete failure diagnostics, and processing container nodes after dependent child nodes during prune operations (PR #609, fixes #608).
...
- Added support for including IGA event certification templates in certification template read, export, and delete flows, along with related variable dependency handling in exports and imports (PR #603).
- Added `onlyCustom` support for full config export/import so callers can limit request type handling to custom request types during IGA migrations (PR #603).
- Added more flexible IGA workflow and certification template operations, including force delete support, separate draft/published workflow deletion options, and updated export behavior for workflows (PR #603).

### Fixed

- Fixed IGA query helpers to return complete result sets instead of truncated pages for workflows, request types, and certification templates (PR #603).
- Fixed request type imports and patches so non-custom request types behave more like the UI, including low-level patch support when needed (PR #603).
...
- \#519: Fixed an error generated by the `frodo admin grant-oauth2-client-admin-privileges` command due to changes in AIC default configuration.
...
[4.1.2]: https://github.com/rockcarver/frodo-lib/compare/v4.1.1...v4.1.2
...
[4.1.1-0]: https://github.com/rockcarver/frodo-lib/compare/v4.1.0...v4.1.1-0
[4.1.0]: https://github.com/rockcarver/frodo-lib/compare/v4.0.2-0...v4.1.0
[4.0.2-0]: https://github.com/rockcarver/frodo-lib/compare/v4.0.1...v4.0.2-0
[4.0.1]: https://github.com/rockcarver/frodo-lib/compare/v4.0.1-6...v4.0.1
[4.0.1-6]: https://github.com/rockcarver/frodo-lib/compare/v4.0.1-5...v4.0.1-6
[4.0.1-5]: https://github.com/rockcarver/frodo-lib/compare/v4.0.1-4...v4.0.1-5
[4.0.1-4]: https://github.com/rockcarver/frodo-lib/compare/v4.0.1-3...v4.0.1-4
[4.0.1-3]: https://github.com/rockcarver/frodo-lib/compare/v4.0.1-2...v4.0.1-3
[4.0.1-2]: https://github.com/rockcarver/frodo-lib/compare/v4.0.1-1...v4.0.1-2
[4.0.1-1]: https://github.com/rockcarver/frodo-lib/compare/v4.0.1-0...v4.0.1-1
[4.0.1-0]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0...v4.0.1-0
...
[4.0.0-45]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-44...v4.0.0-45
[4.0.0-44]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-43...v4.0.0-44
[4.0.0-43]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-42...v4.0.0-43
[4.0.0-42]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-41...v4.0.0-42
[4.0.0-41]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-40...v4.0.0-41
[4.0.0-40]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-39...v4.0.0-40
[4.0.0-39]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-38...v4.0.0-39
[4.0.0-38]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-37...v4.0.0-38
[4.0.0-37]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-36...v4.0.0-37
[4.0.0-36]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-35...v4.0.0-36
[4.0.0-35]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-34...v4.0.0-35
...
[4.0.0-30]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-29...v4.0.0-30
[4.0.0-29]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-28...v4.0.0-29
[4.0.0-28]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-27...v4.0.0-28
[4.0.0-27]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-26...v4.0.0-27
[4.0.0-26]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-25...v4.0.0-26
[4.0.0-25]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-24...v4.0.0-25
[4.0.0-24]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-23...v4.0.0-24
[4.0.0-23]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-22...v4.0.0-23
[4.0.0-22]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-21...v4.0.0-22
...
[4.0.0-17]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-16...v4.0.0-17
[4.0.0-16]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-15...v4.0.0-16
[4.0.0-15]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-14...v4.0.0-15
[4.0.0-14]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-13...v4.0.0-14
[4.0.0-13]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-12...v4.0.0-13
[4.0.0-12]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-11...v4.0.0-12
[4.0.0-11]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-10...v4.0.0-11
[4.0.0-10]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-9...v4.0.0-10
[4.0.0-9]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-8...v4.0.0-9
[4.0.0-8]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-7...v4.0.0-8
[4.0.0-7]: https://github.com/rockcarver/frodo-lib/compare/v4.0.0-6...v4.0.0-7
...
[3.3.4]: https://github.com/rockcarver/frodo-lib/compare/v3.3.4-0...v3.3.4
